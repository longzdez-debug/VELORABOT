...
#[derive(sqlx::FromRow)]
struct Candidate{title:String,price:f64,first_seen_at:DateTime<Utc>}

fn tokens(title:&str)->HashSet<String>{title.to_lowercase().split(|c:char|!c.is_alphanumeric()).filter(|s|s.len()>=2).filter(|s|!matches!(*s,"для"|"и"|"в"|"на"|"с"|"из"|"по"|"the"|"for"|"with"|"gb"|"гб")).map(ToOwned::to_owned).collect()}
...
pub async fn estimate(pool:&PgPool,title:&str,exclude_id:Option<uuid::Uuid>)->(Option<f64>,Option<f64>){
 let wanted=tokens(title);if wanted.is_empty(){return(None,None)}let identity=identity_tokens(title);
 let mut key_parts:Vec<_>=wanted.iter().cloned().collect();key_parts.sort();let cache_key=format!("market:v2:{}",key_parts.join("|"));
 ...
 let rows=sqlx::query_as::<_,Candidate>("SELECT title,price,first_seen_at FROM listings WHERE price IS NOT NULL AND price>0 AND status='active' AND ($1::uuid IS NULL OR id<>$1) ORDER BY first_seen_at DESC LIMIT 1000").bind(exclude_id).fetch_all(pool).await.unwrap_or_default();
 let now=Utc::now();let mut scored=Vec::new();
 for row in rows{let have=tokens(&row.title);let overlap=wanted.intersection(&have).count()as f64;let sim=overlap/wanted.len().max(1)as f64;if overlap==0.0||sim<0.30{continue}
  let have_id=identity_tokens(&row.title);let id_overlap=identity.intersection(&have_id).count()as f64;
  let identity_sim=if identity.is_empty(){sim}else{id_overlap/identity.len().max(1)as f64};
  if !identity.is_empty()&&identity_sim<0.34{continue}
  ...
  let weight=(0.2+sim*1.6)*identity_sim.max(0.35)*exact_bonus*recency;scored.push((row.price,weight));
 }
